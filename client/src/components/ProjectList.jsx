...
function ProjectListItem({
  ...
  useEffect(() => {
    // If this project item becomes selected, ensure its objectives area is shown.
    // If it becomes unselected, it will hide based on isSelected prop in render.
    if (isSelected) {
      setShowObjectivesAndDescription(true);
    } else {
      setShowObjectivesAndDescription(false);
      setShowCreateObjectiveForm(false); // Also hide form if project is deselected
    }
  }, [isSelected]);

  const handleProjectClick = () => {
    onSelectProject(project.id); // This might trigger fetching objectives in App.jsx
    // Toggle local visibility state only if it's already selected.
    // If it's not selected, the useEffect above will handle showing it.
    if (isSelected) {
  ...
  const handleAddObjectiveClick = (e) => {
    e.stopPropagation();
    setShowCreateObjectiveForm(true); // Always show form when "Add Objective" is clicked
  };
  ...
  const handleInternalCreateObjective = async (objectiveData) => {
    try {
      ...
      // Objectives list will be updated by App.jsx fetching them again
    } catch (error) {
      // Error is typically handled/alerted in App.jsx, but could add local feedback
...
function ProjectList({
  ...
  onSelectProject,
  selectedProjectId,
  objectivesByProjectId, // Now an object
  isLoadingObjectives,
  objectiveError,
  onSelectObjective,
  selectedObjectiveId,
    ...
    <ul className="project-list">
      {projects.map(project => (
        <ProjectListItem
          ...
          objectives={objectivesByProjectId[project.id] || []} // Pass objectives for this specific project
          isLoadingObjectives={isLoadingObjectives && project.id === selectedProjectId} // Only show loading for selected project
          objectiveError={project.id === selectedProjectId ? objectiveError : null} // Only show error for selected project
          onSelectObjective={onSelectObjective}
          selectedObjectiveId={selectedObjectiveId}
